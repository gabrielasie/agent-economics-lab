...
join back to the right (funder, invoice) pair by custom_id.
"""
...
def test_custom_id_round_trip() -> None:
    assert decode_custom_id(encode_custom_id("F0", "INV-1")) == ("F0", "INV-1")
...
def test_batch_joins_results_by_custom_id() -> None:
    ...
    responses = {
        encode_custom_id("F0", "INV-7"): '{"apr": 0.11, "rationale": "a"}',
        encode_custom_id("F1", "INV-7"): '{"apr": 0.22, "rationale": "b"}',
    }
